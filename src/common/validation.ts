...
export function Phone(errMsg = '휴대전화 형식에 맞지 않습니다.'): VueDecorator {
    return createValidationDecorator(new RegExp('^01([0|1|6|7|8|9])-?([0-9]{3,4})-?([0-9]{4})$'), errMsg)
}
...
export function Tel(errMsg = '유선전화 형식에 맞지 않습니다.'): VueDecorator {
    return createValidationDecorator(new RegExp('^(0(2|3[1-3]|4[1-4]|5[1-5]|6[1-4]))-?(\\d{3,4})-?(\\d{4})$'), errMsg)
}
